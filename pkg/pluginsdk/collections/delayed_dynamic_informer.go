...
func (d delayedUnstructuredIndex) Lookup(key string) []interface{} {
...
func (d *delayedUnstructuredInformer) AddEventHandler(h cache.ResourceEventHandler) cache.ResourceEventHandlerRegistration {
	...
	reg.hasSynced.Store(ptrToFunc(d.watcher.HasSynced))
...
func (d *delayedUnstructuredInformer) set(inf kclient.Informer[*unstructured.Unstructured]) {
	...
	for _, handler := range d.handlers {
		...
		handler.hasSynced.hasSynced.Store(ptrToFunc(reg.HasSynced))
	}
	d.handlers = nil
...
func ptrToFunc(f func() bool) *func() bool {
	return &f
}
